...
import {
  ...
  awardStatements,
  revokeStatements,
  revokeStatementsForRefIds,
  revokeLikeStatements,
  notifyStatement,
  displayName,
  isAdmin,
  assertCircleReadable,
  parseCursor,
...
export function registerPostsRoutes() {
  ...
  on('GET', '/api/community/posts', false, async (ctx) => {
    ...
    if (follow) {
      where.push('p.user_id IN (SELECT followee_id FROM user_follows WHERE follower_id = ?)')
      params.push(ctx.userId)
    }
    // 知识点讨论流：按 topic_ref 精确匹配（'subjectId|chapterName'）
    if (topicSubject && topicChapter) {
      where.push('p.topic_ref = ?')
      params.push(`${topicSubject}|${topicChapter}`)
    } else if (circleId) {
  ...
  on('POST', '/api/community/posts', true, async (ctx) => {
    ...
    const myPoints = await first<{ points: number }>(
      ctx.env,
      'SELECT COALESCE(points, 0) AS points FROM gamification WHERE user_id = ?',
      ctx.userId
    )
    const pts = myPoints?.points ?? 0
    const cooldown = pts < 500 ? 600 : pts < 1500 ? 300 : 0
    if (cooldown > 0) {
      const last = await first<{ created_at: number }>(
        ctx.env,
  ...
  on('DELETE', '/api/community/posts/:id', true, async (ctx) => {
    ...
    const post = await first<{ user_id: string; image_urls: string }>(
      ctx.env,
      'SELECT user_id, image_urls FROM community_posts WHERE id = ?',
      id
    )
    if (!post) throw new HttpError(404, '帖子不存在')
    ...
    const commentRows = await all<{ id: string; image_urls: string }>(
      ctx.env,
      'SELECT id, image_urls FROM community_comments WHERE post_id = ?',
      id
    )
  ...
  on('POST', '/api/community/posts/:id/comments', true, async (ctx) => {
    ...
    if (typeof b?.parentId === 'string' && b.parentId) {
      parent = await first(
        ctx.env,
        'SELECT user_id, parent_id FROM community_comments WHERE id = ? AND post_id = ? AND is_hidden = 0',
        b.parentId,
        postId
      )
  ...
  on('DELETE', '/api/community/comments/:id', true, async (ctx) => {
    const id = ctx.params.id
    const c = await first<{ post_id: string; user_id: string }>(
      ctx.env,
      'SELECT post_id, user_id FROM community_comments WHERE id = ?',
      id
    )
  ...
  on('PUT', '/api/community/posts/:id/accept', true, async (ctx) => {
    ...
    const post = await first<{ user_id: string; type: string; accepted_answer_id: string | null }>(
      ctx.env,
      'SELECT user_id, type, accepted_answer_id FROM community_posts WHERE id = ? AND is_hidden = 0',
      ctx.params.id
    )
    if (!post) throw new HttpError(404, '帖子不存在')
    ...
    const comment = await first<{ user_id: string; parent_id: string | null }>(
      ctx.env,
      'SELECT user_id, parent_id FROM community_comments WHERE id = ? AND post_id = ? AND is_hidden = 0',
      commentId,
      ctx.params.id
    ...
    if (post.accepted_answer_id) {
      ...
      stmts.push(
        ctx.env.DB.prepare('UPDATE community_comments SET is_accepted = 0 WHERE id = ?').bind(post.accepted_answer_id)
      )
      stmts.push(
        ctx.env.DB.prepare(
          "DELETE FROM community_notifications WHERE type = 'achievement' AND comment_id = ? AND content LIKE '%采纳了你的回答%'"
        ).bind(post.accepted_answer_id)
      )
    }
    if (post.accepted_answer_id === commentId) {
      ...
      stmts.push(
        ctx.env.DB.prepare(
          'UPDATE community_posts SET accepted_answer_id = NULL, is_resolved = 0, updated_at = ? WHERE id = ?'
    ...
    stmts.push(
      ctx.env.DB.prepare(
        'UPDATE community_posts SET accepted_answer_id = ?, is_resolved = 1, updated_at = ? WHERE id = ?'
