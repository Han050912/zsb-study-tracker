...
export function registerMessagesRoutes() {
  ...
  on('GET', '/api/community/messages/conversations', true, async (ctx) => {
    ...
    const unreadRows = await all<{ from_id: string; n: number }>(
      ctx.env,
      'SELECT from_id, COUNT(*) AS n FROM community_messages WHERE to_id = ? AND is_read = 0 GROUP BY from_id',
      ctx.userId
    )
  ...
  on('GET', '/api/community/messages/with/:peerId', true, async (ctx) => {
    ...
    return Response.json({
      messages: items.map((r) => ({
        ...
        isRead: r.from_id === peerId ? true : !!r.is_read,
  ...
  on('POST', '/api/community/messages/:peerId', true, async (ctx) => {
    ...
    await batch(ctx.env, [
      ctx.env.DB.prepare(
        'INSERT INTO community_messages (id, from_id, to_id, content, image_urls, created_at) VALUES (?, ?, ?, ?, ?, ?)'
      ).bind(id, ctx.userId, peerId, content, JSON.stringify(imageUrls), now)
    ])
    return Response.json(
      { id, fromId: ctx.userId, toId: peerId, content, imageUrls, isRead: false, createdAt: now, fromMe: true },
      { status: 201 }
    )
  })
  ...
  on('GET', '/api/community/messages/unread-count', true, async (ctx) => {
    const r = await first<{ n: number }>(
      ctx.env,
      'SELECT COUNT(*) AS n FROM community_messages WHERE to_id = ? AND is_read = 0',
      ctx.userId
    )
