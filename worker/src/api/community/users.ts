...
export function registerUsersRoutes() {
  ...
  on('GET', '/api/community/users/lookup', true, async (ctx) => {
    ...
    const partner =
      u.id === ctx.userId
        ? null
        : await first<{ status: string; to_id: string }>(
            ctx.env,
  ...
  on('GET', '/api/community/users/:id/profile', false, async (ctx) => {
    ...
    const [stats, badges, followers, followedByMe, followsMe, threads, social] = await Promise.all([
      first<{ posts: number; likes: number }>(
        ctx.env,
        `
      ...
      all<{ badge_key: string; awarded_at: number }>(
        ctx.env,
      ...
      first<{ n: number }>(
        ctx.env,
        'SELECT COUNT(*) AS n FROM community_posts WHERE user_id = ? AND is_hidden = 0 AND circle_id IS NULL AND topic_ref IS NULL',
        ctx.params.id
      ),
      first<{ following: number; mutual: number; liked: number }>(
        ctx.env,
        `
  ...
  on('GET', '/api/community/users/:id/stats', true, async (ctx) => {
    ...
    // 总学习时长
    const totalStudy = await first<{ minutes: number; days: number }>(
      ctx.env,
      `
    ...
    // 做题统计
    const problemStats = await first<{ total: number; correct: number; sessions: number }>(
      ctx.env,
      `
      SELECT COALESCE(SUM(total), 0) AS total, COALESCE(SUM(correct), 0) AS correct, COUNT(*) AS sessions
      FROM problem_sessions WHERE user_id = ?`,
      userId
    )

    // 科目分布（学习时长 Top 5 科目）
    const subjectRows = await all<{ subject_id: string; name: string; minutes: number }>(
      ctx.env,
      `
      ...
      WHERE r.user_id = ?
      GROUP BY r.subject_id ORDER BY minutes DESC LIMIT 5`,
      userId
    )
    ...
    return Response.json({
      ...
      problems: {
        ...
        accuracy:
          (problemStats?.total ?? 0) > 0
            ? Math.round(((problemStats?.correct ?? 0) / (problemStats?.total ?? 0)) * 100)
            : 0
      },
  ...
  on('PUT', '/api/community/users/:id/follow', true, async (ctx) => {
    ...
    const existing = await first<{ follower_id: string }>(
      ctx.env,
      'SELECT follower_id FROM user_follows WHERE follower_id = ? AND followee_id = ?',
      ctx.userId,
      targetId
    ...
    if (existing) {
      await batch(ctx.env, [
        ctx.env.DB.prepare('DELETE FROM user_follows WHERE follower_id = ? AND followee_id = ?').bind(
          ctx.userId,
          targetId
        ),
        ctx.env.DB.prepare(
          "DELETE FROM community_notifications WHERE type = 'follow' AND actor_id = ? AND user_id = ?"
  ...
  on('GET', '/api/community/users/:id/posts', false, async (ctx) => {
    ...
    const where = [
      'p.user_id = ?',
      'p.is_hidden = 0 AND (p.is_flagged = 0 OR p.user_id = ?)',
      'p.circle_id IS NULL',
      'p.topic_ref IS NULL'
    ]
    ...
    const rows = await all(
      ctx.env,
      `${POST_SELECT} WHERE ${where.join(' AND ')} ORDER BY p.created_at DESC, p.id DESC LIMIT ?`,
      ...params,
      limit + 1
    )
  ...
  on('GET', '/api/community/me/liked-posts', true, async (ctx) => {
    ...
    const where = [
      ...
      'lk.user_id = ?',
      'p.is_hidden = 0',
      '(p.is_flagged = 0 OR p.user_id = ?)'
    ]
    const params: unknown[] = [ctx.userId, ctx.userId, ctx.userId, ctx.userId]
    if (c) {
      where.push('(lk.created_at < ? OR (lk.created_at = ? AND p.id < ?))')
      params.push(c.ts, c.ts, c.id)
    }
    const sql =
      POST_SELECT.replace('SELECT p.*,', 'SELECT p.*, lk.created_at AS lk_created_at,').replace(
        'FROM community_posts p',
        'FROM community_posts p JOIN community_likes lk ON lk.target_id = p.id'
  ...
  on('GET', '/api/community/users/:id/followers', false, async (ctx) => {
    const target = await first<{ id: string; profile_visibility: string | null }>(
      ctx.env,
      'SELECT u.id, s.profile_visibility FROM users u LEFT JOIN user_settings s ON s.user_id = u.id WHERE u.id = ?',
      ctx.params.id
    )
    ...
    if (c) {
      sql += ' AND (f.created_at < ? OR (f.created_at = ? AND f.follower_id < ?))'
      params.push(c.ts, c.ts, c.id)
    }
  ...
  on('GET', '/api/community/users/:id/following', false, async (ctx) => {
    ...
    const params: unknown[] = [ctx.params.id]
    if (c) {
      sql += ' AND (f.created_at < ? OR (f.created_at = ? AND f.followee_id < ?))'
      params.push(c.ts, c.ts, c.id)
    }
  ...
  on('GET', '/api/community/users/:id/mutual', false, async (ctx) => {
    ...
    const params: unknown[] = [ctx.params.id]
    if (c) {
      sql += ' AND (f1.created_at < ? OR (f1.created_at = ? AND f1.followee_id < ?))'
      params.push(c.ts, c.ts, c.id)
    }
