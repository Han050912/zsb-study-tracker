...
export function registerAdminRoutes() {
  ...
  on('PUT', '/api/admin/posts/:id/pin', true, async (ctx) => {
    ...
    const post = await first<{ is_pinned: number }>(
      ctx.env,
      'SELECT is_pinned FROM community_posts WHERE id = ?',
      ctx.params.id
    )
  ...
  on('PUT', '/api/admin/posts/:id/hide', true, async (ctx) => {
    ...
    const post = await first<{ is_hidden: number }>(
      ctx.env,
      'SELECT is_hidden FROM community_posts WHERE id = ?',
      ctx.params.id
    )
    if (!post) throw new HttpError(404, '帖子不存在')
  ...
  on('PUT', '/api/admin/posts/:id/feature', true, async (ctx) => {
    ...
    const post = await first<{ is_featured: number }>(
      ctx.env,
      'SELECT is_featured FROM community_posts WHERE id = ?',
      ctx.params.id
    )
    if (!post) throw new HttpError(404, '帖子不存在')
  ...
  on('PUT', '/api/admin/posts/:id/daily', true, async (ctx) => {
    ...
    const post = await first<{ is_daily: number }>(
      ctx.env,
      'SELECT is_daily FROM community_posts WHERE id = ?',
      ctx.params.id
    )
    if (!post) throw new HttpError(404, '帖子不存在')
  ...
  on('PUT', '/api/admin/comments/:id/hide', true, async (ctx) => {
    ...
    const c = await first<{ is_hidden: number }>(
      ctx.env,
      'SELECT is_hidden FROM community_comments WHERE id = ?',
      ctx.params.id
    )
  ...
  on('GET', '/api/admin/reports', true, async (ctx) => {
    ...
    const comments = commentIds.length
      ...
      WHERE c.id IN (${commentIds.map(() => '?').join(',')})`,
          ...commentIds
        )
      : []
  ...
  on('PUT', '/api/admin/reports/:id/resolve', true, async (ctx) => {
    ...
    const report = await first<any>(
      ctx.env,
      "SELECT * FROM community_reports WHERE id = ? AND status = 'pending'",
      ctx.params.id
    )
    ...
    if (isMessage && action === 'hide')
      throw new HttpError(400, '私信不支持隐藏操作（仅会话双方可见），请选择删除或驳回')
    const target = await first<any>(
      ctx.env,
      `SELECT ${isMessage ? 'from_id AS user_id' : 'user_id'}${isPost ? ', image_urls' : isMessage ? '' : ', post_id'} FROM ${table} WHERE id = ?`,
      report.target_id
    )
    ...
    if (!target) {
      ...
      stmts.push(
        modLogStatement(ctx.env, ctx.userId, 'resolve-gone', report.target_type, report.target_id, report.id, note)
      )
      stmts.push(
        notifyStatement(ctx.env, {
    ...
    if (action === 'reject') {
      ...
      // （私信不产生通知行，无需撤回；作者与举报人的处理结果通知在下方统一发放）
      if (isMessage) {
        ...
        const commentImgRows = await all<{ image_urls: string }>(
          ctx.env,
          'SELECT image_urls FROM community_comments WHERE post_id = ?',
          report.target_id
        )
    ...
    const [stats, overrides] = await Promise.all([
      all<{ tag: string; count: number }>(
        ...
        `SELECT t.value AS tag, COUNT(*) AS count FROM community_posts p, json_each(p.tags) t
         WHERE p.created_at >= ? AND p.is_hidden = 0 GROUP BY t.value ORDER BY count DESC LIMIT 20`,
        weekAgo
      ),
      all(
        ctx.env,
        'SELECT id, text, tag, action, created_at AS createdAt FROM community_hot_topics ORDER BY created_at DESC'
      )
    ])
