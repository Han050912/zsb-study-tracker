...
<template>
  <div class="max-w-3xl mx-auto px-4 py-6 space-y-5">
    ...
    <div v-if="loading" class="text-center text-slate-400 text-xs py-10">加载中…</div>
    ...
    <template v-else>
      ...
      <div class="card space-y-2">
        ...
        <div v-if="!suggestions.length" class="text-xs text-slate-400 text-center py-4">暂无可推荐的搭子</div>
        <div v-for="s in suggestions" :key="s.userId" class="flex items-center gap-2 text-xs border-b border-slate-50 dark:border-slate-700 last:border-0 py-1.5">
          ...
          <div class="min-w-0">
            ...
            <div class="text-[10px] text-slate-400 truncate">{{ s.reasons.join(' · ') || '缘分推荐' }}</div>
          </div>
      ...
      <div class="card space-y-2">
        ...
        <div v-if="!partners.length" class="text-xs text-slate-400 text-center py-4">还没有搭子，去上方推荐里找一个吧</div>
