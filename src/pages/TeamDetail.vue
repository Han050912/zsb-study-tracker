...
async function handleLeaderLeave() {
  ...
  if (leaveMode.value === 'transfer' && !transferTargetId.value) {
    toast('请选择接任队长')
    return
  }
...
<template>
  <div class="max-w-3xl mx-auto space-y-4">
    ...
    <template v-else-if="team">
      ...
      <div class="card space-y-3">
        ...
        <div
          v-if="team.myRole === 'leader' && !team.isPublic && detail?.inviteCode"
        ...
        >
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span v-if="detail.inviteCodeExpiresAt" class="text-[10px] text-slate-400"
              >有效期至 {{ formatDate(detail.inviteCodeExpiresAt) }}</span
            >
          </div>
        </div>
      </div>
          ...
          <div v-for="m in detail.members" :key="m.userId" class="flex items-center gap-3 py-2">
            ...
            <div v-if="team.myRole === 'leader' && m.role === 'member'" class="flex items-center gap-1.5 shrink-0">
              <button
                ...
                @click="handleTransfer(m.userId, m.userName)"
              >
      ...
      <div class="card">
        <div class="flex items-center gap-2 mb-2">
          ...
          <div class="flex-1"></div>
          <button v-if="team.myRole === 'leader'" class="btn-ghost !text-xs" @click="showCreate = true">
            ＋ 创建挑战
          </button>
        </div>
          ...
          >
            ...
            <div class="flex items-center justify-between mt-2">
              <span class="text-[10px] text-slate-400"
                >团队达标 {{ c.completedCount }}/{{ detail.members.length }} 人</span
              >
    ...
    <div class="space-y-3">
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-1.5 mt-2">
          <button
            v-for="p in TYPE_META[createForm.type].presets"
            :key="p"
  ...
  <Modal :show="!!editingId" title="编辑挑战" @close="editingId = null">
    ...
    <template #footer>
      ...
      <button class="btn-primary" :disabled="editSubmitting" @click="handleEdit">
  ...
  <Modal :show="showEdit" title="编辑小组信息" @close="showEdit = false">
    <div class="space-y-3">
      ...
      <div>
        ...
        <p v-if="editInvalid" class="text-xs text-red-500 mt-1">
          不能低于当前成员数（{{ detail?.members.length }} 人）
        </p>
      </div>
    </div>
    <template #footer>
  ...
  <Modal :show="showLeaveModal" title="退出小组" @close="showLeaveModal = false">
    <div class="space-y-3">
      ...
      <div v-if="leaveMode === 'transfer'" class="space-y-1 pt-1">
        ...
        <div
          v-for="m in detail?.members.filter((x) => x.role === 'member')"
          :key="m.userId"
    ...
    <template #footer>
      ...
      <button
        class="btn-primary"
        :disabled="leaderLeaveSubmitting || (leaveMode === 'transfer' && !transferTargetId)"
