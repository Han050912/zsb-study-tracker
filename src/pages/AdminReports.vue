    ...
    <div v-else class="space-y-3">
      <div v-for="r in reports" :key="r.id" class="card space-y-2">
        <div class="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span>{{ r.targetType === 'post' ? '帖子' : '评论' }}</span>
        ...
        <div v-if="r.target" class="rounded-lg bg-slate-50 dark:bg-slate-700/40 px-3 py-2">
          <div class="text-[10px] text-slate-400 mb-0.5">
            {{ r.target.authorName }} 的{{ r.targetType === 'post' ? '帖子' : '评论' }}
          ...
          <button class="text-[10px] text-primary-500 mt-1" @click="router.push(`/community/post/${r.target.postId}`)">
            查看原帖 →
          </button>
        </div>
        ...
        <div v-if="confirming?.id !== r.id" class="flex gap-2 pt-1">
          <template v-if="r.target">
            <button class="btn-ghost !text-xs" @click="ask(r.id, 'hide')">🙈 隐藏</button>
