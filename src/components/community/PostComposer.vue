 ...
 * 含公式时异步加载 KaTeX chunk 后原地升级。仅预览 Tab 下渲染，编辑态不浪费解析（source 返回空串）。
 */
const previewHtml = useMarkdownHtml(() => (preview.value ? content.value : ''))
...
const placeholder = computed(() =>
  isQuestion.value ? '请描述题目来源、你的思路和卡住的地方…（可贴题目截图）' : '分享你的学习动态…（支持 emoji，可配图）'
)
...
<template>
  <Modal :show="show" :title="isQuestion ? '提问' : '分享到广场'" @close="emit('update:show', false)">
    ...
    <div class="flex items-center gap-1 mb-1.5 flex-wrap">
      ...
      <div class="flex bg-slate-100 dark:bg-slate-700 rounded-md p-0.5 text-[10px]">
        <button
          ...
          :class="
            !preview ? 'bg-white dark:bg-slate-800 font-semibold shadow-sm' : 'text-slate-500 dark:text-slate-400'
          "
          @click="preview = false"
        >
          编辑
    ...
    <div @dragover.prevent @drop.prevent="onDrop">
      ...
      <div
        v-if="preview"
        class="input min-h-[9rem] overflow-y-auto md-body text-sm"
        v-html="previewHtml || '<span style=&quot;color:#94a3b8&quot;>暂无内容</span>'"
      ></div>
    </div>
