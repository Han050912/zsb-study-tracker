...
<template>
  <div v-if="data" class="space-y-3">
    ...
    <div v-if="board?.list.length" class="space-y-1.5">
      ...
      >
        ...
        <div class="flex items-center gap-2 cursor-pointer group" @click="goProfile(e.userId)">
          ...
          <span
            class="font-medium truncate max-w-[7rem] group-hover:text-primary-500"
            :class="e.isMe ? 'text-primary-600 dark:text-primary-300' : ''"
            >{{ e.userName }}</span
          >
        </div>
        <span
    ...
    <div
      v-if="board?.me"
      class="border-t border-slate-100 dark:border-slate-700 pt-2 flex items-center justify-between text-[11px]"
    >
