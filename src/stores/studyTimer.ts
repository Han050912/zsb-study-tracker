...
export const useStudyTimerStore = defineStore('studyTimer', () => {
  ...
  async function completePhase() {
    ...
    if (phase.value === 'focus') {
      ...
      appStore.recordPomodoro(session.value.focusMinutes)
  ...
  async function syncState(state: Phase) {
    ...
    try {
      ...
      if (res.session.status === 'done') { sessionCompleted.value++; finishSession() }
    } catch { /* 同步失败静默 */ }
  }
  ...
  return {
    ...
    display, pomodoroCompleted, sessionCompleted,
    start, pause, endSession, enterSession, finishSession
  }
})
